---
name: power-routing
description: "Orchestrate DipTrace MCP for Power Routing: Plan power routes, via arrays, and copper areas against current, voltage-drop, and thermal targets. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Plan power routes, via arrays, and copper areas against current, voltage-drop, and thermal targets.

Priority: `P0`. Declared maturity: `requires routing and thermal analysis support`. Treat maturity as planning metadata, not runtime proof of availability.
...
- rail current
- voltage-drop and temperature targets
- stackup and copper thickness
- power topology
...
| `route_net` | `route_net` | `exact` | Bounded local 45-degree router without push-and-shove. |
| `add_via` | `add_via` | `exact` | Use only when document capabilities allow it. |
...
3. Identify current paths.
4. Calculate required widths and vias.
5. Plan short routes.
6. Stage vias and copper changes.
7. Check thermal relief and return paths.
8. Evaluate voltage-drop and thermal assumptions.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `get_route_details`, `run_drc`, `run_connectivity_check`, `run_thermal_review`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
- Calculations use stackup data.
- Unknown current remains explicit.
- Bottlenecks are not hidden.
...
> Plan power routing for rail <net> at <A> with <drop/temp> targets; start with a dry-run.
