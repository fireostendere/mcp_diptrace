---
name: differential-pair-router
description: "Orchestrate DipTrace MCP for Differential Pair Router: Route, measure, and tune differential pairs while controlling impedance, skew, and symmetry. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Route, measure, and tune differential pairs while controlling impedance, skew, and symmetry.

Priority: `P0`. Declared maturity: `requires differential-pair routing support`. Treat maturity as planning metadata, not runtime proof of availability.
...
- positive and negative nets
- target impedance
- width and gap
- maximum skew
- layers and via constraints
...
| `get_differential_pair` | `get_differential_pair` | `exact` | Use only when document capabilities allow it. |
...
| `route_diff_pair` | `route_diff_pair` | `exact` | Bounded coupled route; no independent post-route tuning engine. |
| `tune_diff_pair` | — | `missing` | Do not call the target name. Use dependency contract `routing-analysis-tuning-v1`; block only dependent stages. |
| `validate_diff_pair` | `validate_differential_pair` | `alias` | Exact runtime name for differential-pair validation. |
...
3. Verify the pair definition.
4. Validate stackup and geometry.
5. Create a symmetric escape.
6. Generate a coupled route.
7. Measure coupled length, skew, and via symmetry.
8. Tune when supported.
9. Check return path and DRC.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `validate_differential_pair`, `analyze_return_path`, `run_drc`, `run_connectivity_check`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
- `diff-pair route plan`
- `skew and coupling metrics`
...
- Positive and negative nets are mapped correctly.
- Gap and skew are measured along the route.
- Transitions are symmetric or explicitly justified.
...
> Plan a differential-pair route for <P>/<N> under constraints <parameters>; produce a plan and preview first.
