---
name: external-autorouter-orchestrator
description: "Orchestrate DipTrace MCP for External Autorouter Orchestrator: Run a guarded DSN-to-autorouter-to-SES workflow and compare the result with the routing baseline. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# External Autorouter Orchestrator

## Purpose

Run a guarded DSN-to-autorouter-to-SES workflow and compare the result with the routing baseline.

Priority: `P1`. Declared maturity: `requires a DSN/SES adapter`. Treat maturity as planning metadata, not runtime proof of availability.
...
| `export_autorouter_dsn` | `export_autorouter_dsn` | `exact` | Use only when document capabilities allow it. |
| `run_external_autorouter` | `run_external_autorouter` | `exact` | Use only when document capabilities allow it. |
| `inspect_autorouter_result` | `inspect_autorouter_result` | `exact` | Use only when document capabilities allow it. |
| `import_autorouter_ses` | `import_autorouter_ses` | `exact` | Use only when document capabilities allow it. |
| `compare_routing_before_after` | — | `missing` | Do not call the target name. Use dependency contract `semantic-compare-v1`; block only dependent stages. |
...
3. Probe the adapter and version.
4. Export an isolated DSN job.
5. Run a typed whitelisted command.
6. Collect logs and SES artifacts.
7. Import into preview only.
8. Compare DRC, connectivity, vias, and lengths.
9. Apply only an explicitly accepted result.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `inspect_autorouter_result`, `run_drc`, `run_connectivity_check`, `measure_net_lengths`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
- Execution is limited to the adapter whitelist.
- The source design remains unchanged before preview.
- Post-import checks are mandatory.
...
> Run Freerouting for <scope> with budget <time>; import only into preview and compare metrics.
