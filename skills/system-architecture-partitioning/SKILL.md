---
name: system-architecture-partitioning
description: "Orchestrate DipTrace MCP for System Architecture and Functional Partitioning: Define the hardware architecture, signal flow, power flow, and functional-domain boundaries before detailed schematic capture. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Define the hardware architecture, signal flow, power flow, and functional-domain boundaries before detailed schematic capture.
...
- power, cost, and mechanical constraints
...
3. Partition the device into functional blocks.
4. Build signal-flow and power-flow maps.
5. Identify analog, digital, RF, high-current, and safety domains.
6. Assign preliminary net criticality and classes.
7. Compare architectural alternatives and trade-offs.
...
- Every interface has defined levels and direction.
- Every rail has identified sources and consumers.
- Domain boundaries have an engineering rationale.
...
> Use the Design Brief to define hardware architecture, the power tree, interfaces, and critical nets without editing the project.
