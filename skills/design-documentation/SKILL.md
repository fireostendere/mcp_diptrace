...
description: "Orchestrate DipTrace MCP for Design Documentation Generator: Generate revision-linked documentation from the actual schematic, PCB, constraints, BOM, and reports. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Generate revision-linked documentation from the actual schematic, PCB, constraints, BOM, and reports.

Priority: `P1`. Declared maturity: `available now`. Treat maturity as planning metadata, not runtime proof of availability.
...
3. Collect facts through MCP.
4. Document architecture, power, and interfaces.
5. Document stackup, constraints, and layout.
6. Include BOM, testpoints, and bring-up.
7. Include waivers and limitations.
8. Bind outputs to document hash and revision.
...
4. Run the applicable minimal checks: `get_document_info`, `summarize_design`, `get_bom`.
...
- Revision and hash are stated.
- There are no unsupported numeric claims.
- Generated and manual sections are separated.
...
> Generate documentation for the current revision covering architecture, power, interfaces, stackup, BOM, bring-up, and open issues.
