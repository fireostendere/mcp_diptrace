---
name: release-gate
description: "Orchestrate DipTrace MCP for Final Release Gate: Issue a formal pass or blocked decision for a revision from mandatory reviews, waivers, and output manifests. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Issue a formal pass or blocked decision for a revision from mandatory reviews, waivers, and output manifests.

Priority: `P0`. Declared maturity: `depends on review and export skills`. Treat maturity as planning metadata, not runtime proof of availability.
...
| `run_board_review` | `run_board_review` | `exact` | Use only when document capabilities allow it. |
| `run_schematic_review` | `run_schematic_review` | `exact` | Use only when document capabilities allow it. |
| `compare_schematic_to_pcb` | `compare_schematic_to_pcb` | `exact` | Use only when document capabilities allow it. |
| `run_manufacturing_review` | `run_manufacturing_review` | `exact` | Use only when document capabilities allow it. |
| `run_assembly_review` | `run_assembly_review` | `exact` | Use only when document capabilities allow it. |
...
3. Verify identity and hashes.
4. Run the mandatory profile.
5. Review critical and high findings.
6. Verify waiver owner and expiry.
7. Verify outputs and manifests.
8. Issue an evidence-based decision.
...
- PASS is forbidden when a mandatory check is unknown.
- Every waiver has a rationale.
...
> Run the release gate for the current revision under profile <profile> without fixing anything.
