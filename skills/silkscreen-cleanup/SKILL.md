---
name: silkscreen-cleanup
description: "Orchestrate DipTrace MCP for Silkscreen Cleanup: Place RefDes, value, polarity, and custom labels without collisions or loss of readability. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Place RefDes, value, polarity, and custom labels without collisions or loss of readability.

Priority: `P0`. Declared maturity: `requires the silkscreen engine`. Treat maturity as planning metadata, not runtime proof of availability.
...
| `list_board_texts` | `list_board_texts` | `exact` | Use only when document capabilities allow it. |
| `check_silkscreen` | `check_silkscreen` | `exact` | Use only when document capabilities allow it. |
| `plan_silkscreen` | `plan_silkscreen` | `exact` | Use only when document capabilities allow it. |
| `apply_silkscreen_plan` | `apply_silkscreen_plan` | `exact` | Use only when document capabilities allow it. |
| `run_silkscreen_check` | `run_silkscreen_check` | `exact` | Use only when document capabilities allow it. |
...
3. Run the silkscreen check.
4. Build forbidden regions.
5. Generate candidate positions.
6. Score collision, readability, and component association.
7. Preserve locked labels.
8. Show unresolved labels and preview.
9. Rerun checks after application.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `run_silkscreen_check`, `run_drc`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
- No new silkscreen violation is introduced.
- Label association remains unambiguous.
- No label is hidden silently.
...
> Clean silkscreen on <Top/Bottom/all>, preserve locked labels, and show a preview.
