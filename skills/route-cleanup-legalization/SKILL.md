---
name: route-cleanup-legalization
description: "Orchestrate DipTrace MCP for Route Cleanup and Legalization: Reduce unnecessary bends, vias, and detours and legalize geometry without changing connectivity. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# Route Cleanup and Legalization

## Purpose

Reduce unnecessary bends, vias, and detours and legalize geometry without changing connectivity.

Priority: `P1`. Declared maturity: `requires routing optimization support`. Treat maturity as planning metadata, not runtime proof of availability.
...
| `optimize_routes` | — | `missing` | Do not call the target name. Use dependency contract `routing-analysis-tuning-v1`; block only dependent stages. |
| `legalize_routes` | — | `missing` | Do not call the target name. Use dependency contract `routing-analysis-tuning-v1`; block only dependent stages. |
| `replace_trace` | `replace_trace` | `exact` | Use only when document capabilities allow it. |
...
4. Find unnecessary bends, vias, neck-downs, and acute angles.
5. Propose replacements.
6. Legalize geometry.
7. Compare length, via count, and return path.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `get_route_details`, `measure_net_lengths`, `run_drc`, `run_connectivity_check`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
- Length constraints remain satisfied.
- The requested scope is preserved.
...
> Clean routes in <nets/region>, reducing unnecessary vias and bends without changing connectivity.
