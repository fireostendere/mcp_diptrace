...
description: "Orchestrate DipTrace MCP for Critical Net Router: Generate and compare bounded route plans for individual critical nets. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Generate and compare bounded route plans for individual critical nets.

Priority: `P0`. Declared maturity: `requires the routing engine`. Treat maturity as planning metadata, not runtime proof of availability.
...
| `route_connection` | `route_connection` | `exact` | Use only when document capabilities allow it. |
...
3. Verify endpoints and constraints.
4. Query obstacles and corridors.
5. Generate multiple route plans.
6. Score length, bends, vias, and return path.
7. Show previews.
8. Stage the selected plan and run localized validation.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `get_route_details`, `analyze_return_path`, `run_drc`, `run_connectivity_check`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
- There are no shorts or regressions.
- Routing rules are satisfied.
...
> Plan a route for critical net <net> within <region/layers>; show alternatives without committing.
