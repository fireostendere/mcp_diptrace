...
description: "Orchestrate DipTrace MCP for Schematic-to-PCB Consistency: Compare RefDes, nets, values, patterns, fields, and pin-to-pad mapping across schematic, PCB, and libraries. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Compare RefDes, nets, values, patterns, fields, and pin-to-pad mapping across schematic, PCB, and libraries.

Priority: `P0`. Declared maturity: `requires normalized cross-document models`. Treat maturity as planning metadata, not runtime proof of availability.
...
| `compare_schematic_to_pcb` | `compare_schematic_to_pcb` | `exact` | Use only when document capabilities allow it. |
| `validate_pin_pad_mapping` | `validate_pin_pad_mapping` | `exact` | Use only when document capabilities allow it. |
...
3. Match component identities.
4. Match nets and endpoints.
5. Validate pin-to-pad mapping.
6. Compare values, patterns, and fields.
7. Find orphaned and stale data.
8. Prepare a transactional ECO synchronization plan.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `compare_schematic_to_pcb`, `validate_pin_pad_mapping`, `run_connectivity_check`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
- There is no silent remapping.
- Every difference is classified.
- The ECO is transactional.
...
> Compare the schematic and PCB for RefDes, nets, values, patterns, and pin-to-pad mapping; prepare an ECO without committing.
