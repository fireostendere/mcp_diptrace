---
name: design-diff-regression
description: "Orchestrate DipTrace MCP for Semantic Design Diff and Regression Review: Compare revisions by normalized domain model rather than XML text and identify engineering regressions. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# Semantic Design Diff and Regression Review

## Purpose

Compare revisions by normalized domain model rather than XML text and identify engineering regressions.

Priority: `P0`. Declared maturity: `requires normalized domain models`. Treat maturity as planning metadata, not runtime proof of availability.
...
3. Normalize both models.
4. Compare components, nets, rules, placement, routes, and stackup.
5. Classify intentional and unexpected changes.
6. Compare findings and metrics.
...
- XML reordering is not treated as a design change.
- Object identity mapping is stable or ambiguity is reported.
- Regressions are separated from fixes.
...
> Compare revisions <base> and <candidate>, producing a semantic diff and regression report.
