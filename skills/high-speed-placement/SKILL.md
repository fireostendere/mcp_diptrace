---
name: high-speed-placement
description: "Orchestrate DipTrace MCP for High-Speed Component Placement: Place PHYs, clocks, memory, connectors, terminations, and protection according to topology and routing needs. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Place PHYs, clocks, memory, connectors, terminations, and protection according to topology and routing needs.

Priority: `P1`. Declared maturity: `requires placement and review support`. Treat maturity as planning metadata, not runtime proof of availability.
...
| `set_length_constraints` | `set_length_constraints` | `exact` | Use only when document capabilities allow it. |
...
3. Define topology.
4. Place terminations and protection.
5. Minimize stubs and transitions.
6. Reserve routing corridors.
7. Check reference continuity.
8. Prepare differential and length rules.
...
- Terminations are on the topology-correct side.
- Routing corridors are physically realistic.
- The reference-plane plan is explicit.
...
> Optimize placement for interface <type/nets>, including the PHY, connector, ESD, and termination components.
