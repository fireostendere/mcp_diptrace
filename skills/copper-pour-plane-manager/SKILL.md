---
name: copper-pour-plane-manager
description: "Orchestrate DipTrace MCP for Copper Pour and Plane Manager: Analyze pours and planes for priorities, islands, necks, thermal relief, and continuity. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# Copper Pour and Plane Manager

## Purpose

Analyze pours and planes for priorities, islands, necks, thermal relief, and continuity.

Priority: `P1`. Declared maturity: `requires authoritative copper geometry support`. Treat maturity as planning metadata, not runtime proof of availability.
...
| `list_copper_pours` | `list_copper_pours` | `exact` | Use only when document capabilities allow it. |
| `update_copper_pour` | — | `missing` | Do not call the target name. Use dependency contract `copper-geometry-write-v1`; block only dependent stages. |
| `analyze_plane_continuity` | `analyze_plane_continuity` | `exact` | Use only when document capabilities allow it. |
| `rebuild_pour_geometry` | — | `missing` | Do not call the target name. Use dependency contract `copper-geometry-write-v1`; block only dependent stages. |
...
3. Inventory pours.
4. Check nets, priorities, and clearances.
5. Find islands and necks.
6. Evaluate thermal relief.
7. Prepare bounded updates.
8. State refill limitations explicitly.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `list_copper_pours`, `analyze_plane_continuity`, `run_drc`, `run_connectivity_check`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
### `copper-geometry-write-v1`

Authoritative copper-pour update, priority handling and refill.

Required inputs: document identity and SHA-256; pour stable IDs and nets; typed boundary and rule changes; stackup; refill engine version.

Required outputs: refilled authoritative geometry; island and neck metrics; semantic transaction; DRC and connectivity delta; rollback token.

Invariants: boundary-only approximation is not final refill; net assignment is explicit; post-refill review is mandatory.
...
- Islands and necks are identified.
- An approximate refill is never called authoritative.
- Post-refill review is mandatory.
...
> Review pours and planes, identify islands and necks, and prepare a dry-run change plan.
