---
name: return-stitching-via-planner
description: "Orchestrate DipTrace MCP for Return and Stitching Via Planner: Plan return vias at transitions and stitching or fence vias against reference, EMC, and RF constraints. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# Return and Stitching Via Planner

## Purpose

Plan return vias at transitions and stitching or fence vias against reference, EMC, and RF constraints.

Priority: `P1`. Declared maturity: `requires return-path and standalone-via support`. Treat maturity as planning metadata, not runtime proof of availability.
...
| `add_via` | — | `incompatible` | Do not call the target name. Use dependency contract `stitching-via-write-v1`; block only dependent stages. |
...
3. Find transitions and discontinuities.
4. Determine return context.
5. Generate candidates.
6. Check clearance and antipads.
7. Plan spacing.
8. Run DRC and connectivity checks.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `analyze_return_path`, `analyze_plane_continuity`, `run_drc`, `run_connectivity_check`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
### `stitching-via-write-v1`

Standalone net-assigned return, stitching and fence via placement.

Required inputs: document identity and SHA-256; candidate coordinates; net and layer span; via style; clearance and spacing rules.

Required outputs: typed via transaction; net and plane connectivity evidence; clearance preview; return-path delta; rollback token.

Invariants: via net must be explicit; never repurpose a trace-only via primitive; DRC and connectivity checks are mandatory.
...
- Each via is assigned to the correct net.
- No geometry conflict is introduced.
- Spacing assumptions are explicit.
...
> Find candidate return and stitching vias for <region/nets> and prepare a guarded plan.
