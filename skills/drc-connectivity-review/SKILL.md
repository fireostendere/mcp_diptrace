---
name: drc-connectivity-review
description: "Orchestrate DipTrace MCP for DRC and Connectivity Review: Run, group, and prioritize geometry and connectivity findings and regressions. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# DRC and Connectivity Review

## Purpose

Run, group, and prioritize geometry and connectivity findings and regressions.

Priority: `P0`. Declared maturity: `requires the offline DRC engine`. Treat maturity as planning metadata, not runtime proof of availability.
...
4. Run profile checks.
5. Group findings by root cause.
6. Compare against the baseline.
7. Separate new, existing, and waived findings.
8. Produce a remediation order.
...
- There is no hidden suppression.
- Rule provenance and measured delta are present.
...
> Run full DRC and connectivity review, compare it with the baseline, and make no changes.
