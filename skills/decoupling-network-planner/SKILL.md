---
name: decoupling-network-planner
description: "Orchestrate DipTrace MCP for Decoupling Network Planner: Define local, group, and bulk capacitors and verify their actual connectivity and placement relative to power pins. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# Decoupling Network Planner

## Purpose

Define local, group, and bulk capacitors and verify their actual connectivity and placement relative to power pins.

Priority: `P0`. Declared maturity: `partially available now`. Treat maturity as planning metadata, not runtime proof of availability.
...
- IC power pins and rails
- datasheets
- edge rates and clock domains
- PDN targets when available
...
3. Map power pins to rails.
4. Extract manufacturer recommendations.
5. Assign a role to every capacitor.
6. Verify rating, dielectric, package, and assumptions.
7. Derive pin-specific placement constraints.
8. Measure loop geometry after placement.
...
- `decoupling plan`
- `capacitor-to-pin mapping`
- `placement constraints`
- `missing and duplicate findings`
...
- Every capacitor has an explicit function.
- Datasheet requirements override generic guidance.
- Layout conclusions use measured geometry.
...
> Review decoupling for <IC/rail>, produce a capacitor-to-pin plan, and assess physical placement.
