---
name: failure-analysis-rework
description: "Orchestrate DipTrace MCP for Failure Analysis and Rework Planning: Rank failure hypotheses from symptoms and design evidence, select diagnostic measurements, and prepare rework or an ECO. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# Failure Analysis and Rework Planning

## Purpose

Rank failure hypotheses from symptoms and design evidence, select diagnostic measurements, and prepare rework or an ECO.

Priority: `P1`. Declared maturity: `available now as read-only analysis`. Treat maturity as planning metadata, not runtime proof of availability.
...
4. Form causal hypotheses.
5. Correlate power, connectivity, and layout evidence.
6. Choose discriminating measurements.
7. Evaluate rework feasibility.
8. Prepare an ECO only after root cause is supported.
...
- Measurements discriminate between hypotheses.
- Rework is never executed automatically.
...
> Analyze failure <symptoms/measurements>, rank hypotheses, and produce diagnostic and rework plans.
