---
name: dfa-review
description: "Orchestrate DipTrace MCP for Design for Assembly Review: Review spacing, access, polarity, orientation, paste, fiducials, and assembly-process risks. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Review spacing, access, polarity, orientation, paste, fiducials, and assembly-process risks.

Priority: `P0`. Declared maturity: `requires versioned assembly profiles`. Treat maturity as planning metadata, not runtime proof of availability.
...
3. Check spacing and access.
4. Check polarity and pin-1 markings.
5. Evaluate tombstoning heuristics.
6. Check paste geometry.
7. Check orientation and sides.
8. Check fiducials, tooling, and assembly notes.
...
- Checks are process-specific.
- Markings are verified.
- DNP state and variants are included.
...
> Run a DFA review for process <SMT/THT/mixed> and assembler profile <assembler>.
