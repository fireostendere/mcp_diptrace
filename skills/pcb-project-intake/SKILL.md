...
description: "Orchestrate DipTrace MCP for PCB Project Requirements Intake: Convert an unstructured device description into a verifiable PCB Design Brief, an initial constraint set, and explicit open questions. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# PCB Project Requirements Intake

## Purpose

Convert an unstructured device description into a verifiable PCB Design Brief, an initial constraint set, and explicit open questions.
...
- power rails, interfaces, frequencies, currents, and voltages
- size, cost, manufacturing, environment, and certification constraints
...
| `summarize_design` | `summarize_design` | `exact` | Use only when document capabilities allow it. |
...
4. Assign each requirement an ID, priority, source, and verification method.
5. Identify contradictions and blocking unknowns.
6. Derive electrical, mechanical, manufacturing, and test constraints.
7. Prepare a traceable handoff to downstream skills.
...
- Unknowns block only dependent stages.
...
> Analyze the device requirements <description>; produce a PCB Design Brief, constraints, risks, and open questions without changing the design.
