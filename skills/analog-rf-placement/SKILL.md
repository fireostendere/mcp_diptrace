---
name: analog-rf-placement
description: "Orchestrate DipTrace MCP for Analog and RF Placement: Plan low-noise analog and RF placement, grounding, matching networks, antenna keepouts, and isolation. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# Analog and RF Placement

## Purpose

Plan low-noise analog and RF placement, grounding, matching networks, antenna keepouts, and isolation.

Priority: `P1`. Declared maturity: `requires placement and geometry support`. Treat maturity as planning metadata, not runtime proof of availability.
...
- analog or RF chain
- sensitivity and frequency bands
- antenna and shielding constraints
- ground strategy
...
3. Identify sensitive and noisy nodes.
4. Define domains without breaking return paths.
5. Place filtering and matching networks.
6. Enforce antenna keepouts.
7. Check guards, shields, and stitching.
8. Derive constraints.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `analyze_placement`, `analyze_return_path`, `run_component_clearance_check`, `run_drc`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
- Separation does not break return paths.
- Antenna keepouts are respected.
- Matching placement follows topology.
...
> Review and optimize placement for analog/RF block <description>.
