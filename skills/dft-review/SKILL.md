---
name: dft-review
description: "Orchestrate DipTrace MCP for Design for Test Review: Review test coverage, programming and debug access, probe geometry, and production-test feasibility. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Review test coverage, programming and debug access, probe geometry, and production-test feasibility.

Priority: `P0`. Declared maturity: `requires testpoint and testability review support`. Treat maturity as planning metadata, not runtime proof of availability.
...
| `list_testpoints` | `list_testpoints` | `exact` | Use only when document capabilities allow it. |
| `analyze_programming_interface` | — | `missing` | Do not call the target name. Use dependency contract `domain-analyzer-v1`; block only dependent stages. |
| `run_testability_review` | `run_testability_review` | `exact` | Use only when document capabilities allow it. |
...
3. Measure coverage with an explicit denominator.
4. Check power and ground injection.
5. Check programming and debug access.
6. Check probe access.
7. Check measurement isolation.
8. Prepare a missing-coverage plan.
...
- The coverage denominator is defined.
- Inaccessible points are not counted as covered.
- Production and debug coverage remain distinct.
...
> Run a DFT review for strategy <description> and prepare a plan for missing testpoints.
