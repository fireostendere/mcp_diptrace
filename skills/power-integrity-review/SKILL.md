---
name: power-integrity-review
description: "Orchestrate DipTrace MCP for Power Integrity Review: Review the PDN from rail budgets and decoupling through plane necks, vias, and high-current loops. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Review the PDN from rail budgets and decoupling through plane necks, vias, and high-current loops.
...
| `analyze_decoupling` | — | `missing` | Do not call the target name. Use dependency contract `domain-analyzer-v1`; block only dependent stages. |
| `run_external_solver` | — | `missing` | Do not call the target name. Use dependency contract `solver-adapter-v1`; block only dependent stages. |
...
3. Check rails and budgets.
4. Check decoupling.
5. Evaluate path proxies.
6. Find necks and via bottlenecks.
7. Check loops and return paths.
8. Prepare PDN simulation inputs.
...
Typed external SI, PI, impedance or thermal solver execution.

Required inputs: solver adapter and version; bounded exported geometry; material and boundary-condition provenance; analysis profile; time and resource budget.

Required outputs: typed job ID; logs and artifacts; solver result with units; model limitations; resource URIs.

Invariants: fixed typed argv and shell=false; no solver claim without an actual result; analytical estimates remain distinct.
...
- Loads and operating modes are stated.
- Heuristics are not presented as full simulation.
- Each bottleneck is linked to a rail.
...
> Run a PI/PDN review of rails <nets>, including decoupling, planes, and vias.
