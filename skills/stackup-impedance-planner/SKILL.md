---
name: stackup-impedance-planner
description: "Orchestrate DipTrace MCP for Stackup and Impedance Planner: Select stackup alternatives, reference planes, and preliminary controlled-impedance trace geometry. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# Stackup and Impedance Planner

## Purpose

Select stackup alternatives, reference planes, and preliminary controlled-impedance trace geometry.

Priority: `P0`. Declared maturity: `requires the impedance engine`. Treat maturity as planning metadata, not runtime proof of availability.
...
- current and plane requirements
...
| `suggest_trace_geometry_for_impedance` | `suggest_trace_geometry_for_impedance` | `exact` | Use only when document capabilities allow it. |
| `set_stackup` | — | `missing` | Do not call the target name. Use dependency contract `board-structure-write-v1`; block only dependent stages. |
| `set_diff_pair_rules` | `set_diff_pair_rules` | `exact` | Use only when document capabilities allow it. |
...
3. Check stackup completeness.
4. Propose stackup alternatives.
5. Assign signal and reference layer pairs.
6. Calculate widths and gaps.
7. Check manufacturability and sensitivity.
8. Prepare rule updates and fabricator questions.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `get_stackup`, `validate_impedance_constraints`, `get_design_rules`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
- Unknown dielectric constants and thicknesses are not substituted.
- Analytical estimates and solver-backed results remain distinct.
- Manufacturing tolerances are included.
...
> Plan a stackup for <requirements>, calculate widths and gaps, and prepare rule updates without committing.
