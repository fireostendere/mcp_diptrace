---
name: routing-priority-planner
description: "Orchestrate DipTrace MCP for Routing Priority Planner: Determine routing sequence, batches, and corridors from criticality, topology, and congestion. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# Routing Priority Planner

## Purpose

Determine routing sequence, batches, and corridors from criticality, topology, and congestion.

Priority: `P0`. Declared maturity: `requires routing analysis support`. Treat maturity as planning metadata, not runtime proof of availability.
...
3. Classify nets.
4. Identify escape routing, clocks, differential pairs, and power.
5. Analyze congestion.
6. Build routing batches.
7. Assign quality gates.
8. Explain the ordering.
...
- The ordering has an engineering rationale.
- Critical nets are handled early.
- Reserved corridors are legal.
...
> Plan the routing order for the active board and group nets into batches without routing them.
