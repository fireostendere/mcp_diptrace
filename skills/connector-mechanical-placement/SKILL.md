---
name: connector-mechanical-placement
description: "Orchestrate DipTrace MCP for Connector and Mechanical Placement: Place connectors, controls, LEDs, antennas, and sensors against enclosure datums and mating constraints. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# Connector and Mechanical Placement

## Purpose

Place connectors, controls, LEDs, antennas, and sensors against enclosure datums and mating constraints.

Priority: `P0`. Declared maturity: `requires geometry and placement write support`. Treat maturity as planning metadata, not runtime proof of availability.
...
| `move_components` | `move_components` | `exact` | Use only when document capabilities allow it. |
| `rotate_components` | `rotate_components` | `exact` | Use only when document capabilities allow it. |
| `lock_components` | `lock_components` | `exact` | Use only when document capabilities allow it. |
...
3. Define fixed coordinates.
4. Verify orientation and mating envelope.
5. Check enclosure clearance.
6. Create routing corridors and keepouts.
7. Check labels and pin-1 orientation.
8. Lock objects only after approval.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `run_component_clearance_check`, `run_drc`, `get_board_model`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
- Positions reference the stated datum.
- The mating envelope is included.
- Objects are locked only after preview and confirmation.
...
> Place connectors <RefDes> at the supplied mechanical coordinates and verify access and routing corridors.
