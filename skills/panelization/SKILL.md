---
name: panelization
description: "Orchestrate DipTrace MCP for Panelization: Plan panel arrangements, rails, tabs or V-score, fiducials, tooling, and breakaway features. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Plan panel arrangements, rails, tabs or V-score, fiducials, tooling, and breakaway features.

Priority: `P1`. Declared maturity: `requires panelization support or an adapter`. Treat maturity as planning metadata, not runtime proof of availability.
...
3. Check edge constraints.
4. Generate panel arrangements.
5. Evaluate utilization and orientation.
6. Place rails, fiducials, and tooling.
7. Add breakaway features.
8. Check component and copper clearance.
9. Create previews and notes.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `run_manufacturing_geometry_check`, `run_assembly_review`, `run_drc`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
Typed panel geometry, breakaway, fiducial and tooling-hole planning and mutation.

Required inputs: board and panel identities; versioned fabrication and assembly profile; breakaway method; orientation constraints; scope and SHA-256.

Required outputs: ranked panel plans; utilization metrics; dimensioned preview; semantic transaction; manufacturing and assembly validation.

Invariants: respect component and copper edge clearance; preview before commit; breakaway safety and orientation are explicit.
...
- Breakaway features are safe.
- Orientation is compatible with assembly.
...
> Plan a panel for <size/method/fabricator>, showing ranked arrangements and utilization.
