...
Review BOM completeness, consistency, variants, DNP state, MPNs, and package mapping.
...
| `compare_bom_to_design` | `compare_bom_to_design` | `exact` | Use only when document capabilities allow it. |
| `validate_mpn_consistency` | `validate_mpn_consistency` | `exact` | Use only when document capabilities allow it. |
...
3. Compare the BOM with the design.
4. Check required fields.
5. Check value, package, and MPN consistency.
6. Apply DNP and variant rules.
7. Find duplicates.
8. Evaluate sourcing risk only through an explicit adapter.
...
- All RefDes are accounted for.
- Substitutions are checked parametrically.
- Variant logic is reproducible.
...
> Review the BOM for required fields, variants, DNP state, and package/MPN consistency.
