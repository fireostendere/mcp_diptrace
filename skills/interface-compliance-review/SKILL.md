---
name: interface-compliance-review
description: "Orchestrate DipTrace MCP for Interface Compliance Review: Review USB, Ethernet, HDMI, CAN, RS-485, UART, I2C, SPI, PCIe, MIPI, and other interfaces against parameterized profiles. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Review USB, Ethernet, HDMI, CAN, RS-485, UART, I2C, SPI, PCIe, MIPI, and other interfaces against parameterized profiles.

Priority: `P1`. Declared maturity: `requires versioned interface profiles`. Treat maturity as planning metadata, not runtime proof of availability.
...
- interface type, revision, and data rate
- transceiver and connector datasheets
- schematic, PCB, and stackup
...
| `analyze_interface` | — | `missing` | Do not call the target name. Use dependency contract `domain-analyzer-v1`; block only dependent stages. |
| `analyze_differential_pair` | `analyze_differential_pair` | `exact` | Use only when document capabilities allow it. |
| `analyze_return_path` | `analyze_return_path` | `exact` | Use only when document capabilities allow it. |
| `calculate_impedance` | `calculate_impedance` | `exact` | Verified analytical microstrip estimate only; distinguish it from field-solver or fabricator results. |
| `get_route_details` | `get_route_details` | `exact` | Use only when document capabilities allow it. |
...
3. Select a versioned interface checklist.
4. Check pinout, levels, terminations, pulls, ESD, and common-mode provisions.
5. Check placement topology.
6. Check routing, impedance, length, and return path.
7. Identify solver and certification checks that remain external.
...
Deterministic interface, decoupling, reset, boot and programming analyzers driven by versioned profiles and datasheet facts.

Required inputs: document identity and SHA-256; profile or datasheet revision; typed component and net roles; operating modes; explicit unknowns.

Required outputs: profile version; object-linked findings; checked and skipped rules; confidence; resource URI.

Invariants: datasheet facts outrank generic heuristics; profile version is mandatory; never claim certification or complete coverage from heuristics.
...
- The checklist matches the stated standard revision.
- Heuristic evidence is not presented as certification.
- Data rate and assumptions are explicit.
...
> Review interface <type/revision> across the schematic and PCB, including protection, routing, and return path.
