---
name: eco-change-impact
description: "Orchestrate DipTrace MCP for ECO Change Impact Analysis: Formalize a change request, derive its impact graph, and prepare a guarded semantic transaction. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Formalize a change request, derive its impact graph, and prepare a guarded semantic transaction.

Priority: `P0`. Declared maturity: `requires semantic transactions`. Treat maturity as planning metadata, not runtime proof of availability.
...
| `query_objects` | `query_objects` | `exact` | Use only when document capabilities allow it. |
...
3. Formalize the change.
4. Find impacted objects, rules, and outputs.
5. Build the dependency graph.
6. Create typed semantic operations.
7. Assess regression risks.
8. Apply only after preview and checks.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `run_connectivity_check`, `run_drc`, `run_erc`, `compare_schematic_to_pcb`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
> Prepare an ECO for <change>, showing its impact graph and semantic plan without committing.
