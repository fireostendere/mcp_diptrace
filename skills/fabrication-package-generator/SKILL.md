---
name: fabrication-package-generator
description: "Orchestrate DipTrace MCP for Fabrication Package Generator: Generate, verify, and package fabrication outputs reproducibly through a confirmed native exporter. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# Fabrication Package Generator

## Purpose

Generate, verify, and package fabrication outputs reproducibly through a confirmed native exporter.
...
| `export_fabrication_outputs` | — | `incompatible` | Do not call the target name. Use dependency contract `native-export-v1`; block only dependent stages. |
| `run_manufacturing_review` | `run_manufacturing_review` | `conditional` | Use the runtime tool only for its bounded subset. For the full request use `profile-adapter-v1`. |
| `get_stackup` | `get_stackup` | `exact` | Use only when document capabilities allow it. |
| `get_job_result` | `get_job_result` | `exact` | Use only when document capabilities allow it. |
...
4. Export layers, drills, and drawings only through a confirmed adapter.
5. Verify the layer set and drills.
6. Create stackup and impedance notes.
7. Create a manifest and file hashes.
8. Verify with an independent parser or renderer.
...
4. Run the applicable minimal checks: `run_manufacturing_review`, `run_manufacturing_geometry_check`, `get_stackup`.
...
- Every file has a hash and purpose.
- No stale revision file is included.
...
> Generate and verify a fabrication package for profile <fabricator>, including a complete manifest.
