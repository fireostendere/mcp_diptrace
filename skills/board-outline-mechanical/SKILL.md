---
name: board-outline-mechanical
description: "Orchestrate DipTrace MCP for Board Outline and Mechanical Constraints: Create and validate board outlines, cutouts, holes, keepouts, and the mechanical envelope. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Create and validate board outlines, cutouts, holes, keepouts, and the mechanical envelope.

Priority: `P0`. Declared maturity: `requires geometry and board-structure write support`. Treat maturity as planning metadata, not runtime proof of availability.
...
- enclosure drawing or dimensions
...
| `run_component_clearance_check` | `run_component_clearance_check` | `exact` | Use only when document capabilities allow it. |
...
3. Define the datum and coordinate system.
4. Validate outline and cutouts.
5. Place holes.
6. Create keepouts.
7. Check edge clearance and accessibility.
8. Produce a dimensioned overlay preview.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `get_board_model`, `run_component_clearance_check`, `run_drc`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
- There is no hidden scaling.
- Critical dimensions appear in the preview.
...
> Validate or plan the board outline from mechanical data <description>, including keepouts and a preview.
