---
name: return-path-review
description: "Orchestrate DipTrace MCP for Return Path Review: Review reference-plane continuity, splits, voids, transitions, and return-via coverage. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Review reference-plane continuity, splits, voids, transitions, and return-via coverage.

Priority: `P0`. Declared maturity: `requires the return-path engine`. Treat maturity as planning metadata, not runtime proof of availability.
...
3. Map segments to reference layers.
4. Find splits and voids.
5. Check transitions and return vias.
6. Estimate return-current detours.
7. Suggest routing and stitching changes.
8. Identify solver cases.
...
- References are derived from the stackup.
- Split crossings include coordinates.
- Recommendations do not create plane necks.
...
> Run a return-path review for <nets/classes>, identifying split crossings and poor transitions.
