---
name: pcb-design-orchestrator
description: "Orchestrate DipTrace MCP for End-to-End PCB Design Orchestrator: Coordinate specialized skills, dependency sequencing, and quality gates from requirements through release. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# End-to-End PCB Design Orchestrator

## Purpose

Coordinate specialized skills, dependency sequencing, and quality gates from requirements through release.
...
| `get_job_status` | `get_job_status` | `exact` | Requires a jobid and cannot discover a job implicitly. |
...
3. Determine the actual project phase.
4. Build the dependency graph.
5. Assign skills and non-overlapping scopes.
6. Define quality gates.
7. Collect blockers and regression deltas.
8. Prevent unresolved critical findings from being bypassed.
...
- There are no hidden write operations.
- Every task has an explicit completion criterion.
- A resumed run starts from verified current state.
...
> Assess the project state and plan the required PCB skills through release without applying changes.
