---
name: bringup-plan
description: "Orchestrate DipTrace MCP for Board Bring-Up Plan: Produce a safe, staged bring-up procedure with expected measurements and stop conditions. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Produce a safe, staged bring-up procedure with expected measurements and stop conditions.

Priority: `P0`. Declared maturity: `available now`. Treat maturity as planning metadata, not runtime proof of availability.
...
3. Define unpowered checks.
4. Define short-circuit and resistance checks.
5. Create a current-limited power sequence.
6. Assign measurement points and ranges.
7. Add programming and peripheral stages.
8. Define stop conditions and logging.
...
- Every hazardous step has a stop condition.
- Referenced testpoints are verified.
...
> Create a bring-up plan for the current board using lab equipment <list>.
