---
name: placement-optimizer-legalizer
description: "Orchestrate DipTrace MCP for Placement Optimizer and Legalizer: Improve placement incrementally within a bounded scope without discarding engineering intent. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# Placement Optimizer and Legalizer

## Purpose

Improve placement incrementally within a bounded scope without discarding engineering intent.

Priority: `P1`. Declared maturity: `requires the placement engine`. Treat maturity as planning metadata, not runtime proof of availability.
...
- target selector or region
- fixed and locked objects
- objectives and weights
- allowed rotations and sides
- time budget
...
| `analyze_placement` | `analyze_placement` | `exact` | Use only when document capabilities allow it. |
| `plan_component_placement` | `plan_component_placement` | `exact` | Use only when document capabilities allow it. |
| `score_placement` | `score_placement` | `exact` | Use only when document capabilities allow it. |
| `legalize_component_placement` | `legalize_component_placement` | `exact` | Use only when document capabilities allow it. |
| `apply_component_placement_plan` | `apply_component_placement_plan` | `exact` | Use only when document capabilities allow it. |
...
4. Generate incremental candidates.
5. Optimize with a deterministic seed.
6. Legalize overlaps and edge violations.
7. Compare congestion and score breakdowns.
8. Reject any critical-constraint degradation.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `analyze_placement`, `run_component_clearance_check`, `run_drc`, `run_connectivity_check`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
- Scope is preserved.
- Locked objects remain unchanged.
- Variants use comparable metrics.
...
> Optimize placement <selector> in <region> using weights <parameters>; show three ranked variants.
