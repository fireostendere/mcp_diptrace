---
name: multi-agent-pcb-coordinator
description: "Orchestrate DipTrace MCP for Multi-Agent PCB Coordinator: Delegate PCB work packages to specialized agents while controlling scopes, conflicts, and independent review. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# Multi-Agent PCB Coordinator

## Purpose

Delegate PCB work packages to specialized agents while controlling scopes, conflicts, and independent review.

Priority: `P1`. Declared maturity: `depends on the agent runtime`. Treat maturity as planning metadata, not runtime proof of availability.
...
3. Split the goal into work packages.
4. Assign roles and non-overlapping scopes.
5. Define read and write policy.
6. Collect structured outputs.
7. Assign an independent reviewer.
8. Merge only compatible transactions.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `get_findings`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
- Assumption conflicts are detected.
...
> Partition task <goal> among PCB agents and assign scopes and quality gates.
