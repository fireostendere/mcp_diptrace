---
name: functional-block-placement
description: "Orchestrate DipTrace MCP for Functional Block Placement: Perform floorplanning from signal flow, power flow, criticality, thermal constraints, and routing corridors. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Perform floorplanning from signal flow, power flow, criticality, thermal constraints, and routing corridors.

Priority: `P0`. Declared maturity: `requires the placement engine`. Treat maturity as planning metadata, not runtime proof of availability.
...
| `generate_placement_candidates` | `generate_placement_candidates` | `exact` | Bounded local candidates, not global placement. |
...
3. Classify fixed and movable objects.
4. Assign regions.
5. Define proximity and separation constraints.
6. Generate candidates.
7. Score wirelength, crossings, density, and congestion.
8. Stage a legal placement plan.
...
- No new overlap or edge violation is introduced.
...
> Plan floorplacement for block <RefDes> in region <region>; show ranked variants and score breakdowns.
