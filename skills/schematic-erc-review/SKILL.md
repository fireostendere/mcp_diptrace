...
description: "Orchestrate DipTrace MCP for Schematic ERC and Engineering Review: Combine deterministic ERC with explainable engineering heuristics while keeping their evidence classes distinct. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# Schematic ERC and Engineering Review

## Purpose

Combine deterministic ERC with explainable engineering heuristics while keeping their evidence classes distinct.
...
3. Run ERC.
4. Separate intentional no-connects.
5. Check power sources, inputs, and output conflicts.
6. Check floating inputs, labels, hierarchy, and patterns.
7. Add confidence-scored heuristic findings.
...
- Errors are separated from recommendations.
- Findings include object references and rationale.
- Ambiguous behavior is never corrected automatically.
...
> Run full ERC and an engineering review of the active schematic without changing it.
