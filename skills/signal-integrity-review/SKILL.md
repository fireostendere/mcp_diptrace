...
description: "Orchestrate DipTrace MCP for Signal Integrity Review: Perform an explainable SI review of topology, impedance, terminations, stubs, crosstalk, and return paths. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Perform an explainable SI review of topology, impedance, terminations, stubs, crosstalk, and return paths.
...
| `analyze_controlled_impedance_nets` | `analyze_controlled_impedance` | `alias` | Preliminary analytical microstrip review only; not a field solver. |
...
| `export_si_geometry` | — | `missing` | Do not call the target name. Use dependency contract `solver-adapter-v1`; block only dependent stages. |
...
3. Classify transmission-line nets.
4. Check topology and termination.
5. Check stubs, vias, and length.
6. Estimate parallelism and crosstalk risk.
7. Check return paths.
8. Identify simulation candidates.
...
- Edge rate is included.
- Heuristics carry confidence.
- An eye-diagram pass is never claimed without simulation.
...
> Run an SI review of critical nets <list>, including stubs, crosstalk risk, and return paths.
