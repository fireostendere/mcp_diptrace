...
description: "Orchestrate DipTrace MCP for Testpoint Planner: Select nets and place testpoints against coverage targets and fixture geometry. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Select nets and place testpoints against coverage targets and fixture geometry.

Priority: `P0`. Declared maturity: `requires testpoint write support`. Treat maturity as planning metadata, not runtime proof of availability.
...
| `find_testpoint_candidates` | `find_testpoint_candidates` | `exact` | Use only when document capabilities allow it. |
| `add_testpoints` | `add_testpoints` | `exact` | Use only when document capabilities allow it. |
...
5. Find candidates.
6. Check probe access and spacing.
7. Cover power, ground, and programming interfaces.
8. Compare the coverage delta.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `review_testpoint_coverage`, `run_drc`, `run_connectivity_check`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
- Coverage has a defined denominator.
- Testpoints are physically accessible.
- No DRC violation is introduced.
...
> Plan testpoints for <coverage> on side <side> with probe constraints <parameters>.
