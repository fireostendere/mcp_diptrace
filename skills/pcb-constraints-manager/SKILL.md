---
name: pcb-constraints-manager
description: "Orchestrate DipTrace MCP for PCB Constraint Manager: Normalize requirements into one versioned set of ERC, DRC, net-class, differential-pair, length, mechanical, and manufacturing constraints. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---
...
Normalize requirements into one versioned set of ERC, DRC, net-class, differential-pair, length, mechanical, and manufacturing constraints.

Priority: `P0`. Declared maturity: `requires extended MCP support`. Treat maturity as planning metadata, not runtime proof of availability.
...
- PCB Design Brief
- stackup and fabricator profile
- interface standards
...
| `update_net_class_rules` | `update_net_class_rules` | `exact` | Use only when document capabilities allow it. |
...
3. Collect current rules.
4. Map rules to requirements.
5. Identify gaps, conflicts, and infeasible constraints.
6. Produce a provenance-linked constraint manifest.
7. Prepare a semantic dry-run of rule updates.
...
6. Re-read document identity and SHA-256, then rerun every applicable regression check. For schematic or mixed documents run `run_erc`; for PCB or mixed documents run `run_drc`; for schematic, PCB or mixed documents always run `run_connectivity_check`. Also run the affected domain checks from: `get_design_rules`, `run_drc`, `run_connectivity_check`. Record a check as `not_applicable` only when the document kind makes it invalid; an unavailable applicable check blocks success.
...
> Normalize the project constraints, compare them with current DipTrace rules, and prepare a dry-run correction plan.
