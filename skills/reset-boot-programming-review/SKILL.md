---
name: reset-boot-programming-review
description: "Orchestrate DipTrace MCP for Reset, Boot, and Programming Review: Review reset circuits, strap pins, boot modes, programming, debug, and recovery access. Use when the user requests this workflow for a DipTrace schematic, PCB, library, review, or explicitly bounded project scope. Start with capability discovery; never simulate unavailable tools or write without a guarded preview."
---

# Reset, Boot, and Programming Review

## Purpose

Review reset circuits, strap pins, boot modes, programming, debug, and recovery access.
...
| `analyze_reset_and_boot` | — | `missing` | Do not call the target name. Use dependency contract `domain-analyzer-v1`; block only dependent stages. |
...
3. Identify reset, boot, and debug pins.
4. Check pulls, timing, and voltage domains.
5. Check peripheral multiplexing conflicts.
6. Check connectors and testpoints.
7. Verify production and recovery scenarios.
...
- Every boot state is defined.
- Debug access does not conflict with the application.
...
> Review reset, boot straps, and programming/debug access for <MCU/SoC> in the schematic and PCB.
