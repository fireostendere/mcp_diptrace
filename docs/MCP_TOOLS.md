...
- `plan_schematic_placement_repair` — bounded route-feedback placement repair combined with selective atomic affected-net reroute, stored as one dependency-safe plan. Works on already-wired schematics: affected explicit wire geometry is deleted and selectively replanned. `moves` coordinates are built on the document's current placement and enforced as fixed constraints — repair may improve other parts but never displaces the requested positions;
- `apply_schematic_placement_repair_plan` — stages or commits that stored plan through the ordinary expected-SHA transaction path (`dry_run` defaults to true). Plans exceeding the 100-operation transaction limit are rejected at planning time, not only at apply time.
