...
### 12 atomic reroute with pre-existing ugly geometry/readability warning

- Code identity `ceb64c6`; native-canonical case-11 re-export is the seed. A public guarded transaction creates controlled precondition `c12_bad.dchxml`, SHA-256 `51ecd268bde6b7ef10f06074ea3c528cabc7516fa2a90286069f905ff4696a0e`: the OUT branch remains orthogonal and electrically correct but has a 3-bend, `92.38 mm` detour against `32.38 mm` direct distance. The internal planner is used only diagnostically after public authoring and reports `move_endpoint_blocks_closer`, ratio `2.85 > 2.5`; obstacle/overlap/crossing/self-intersection/diagonal counts and ERC/connectivity findings are zero.
- One public atomic repair transaction performs `delete_wire -> move_components -> add_wire` on OUT only. Candidate `c12_01.dchxml`, SHA-256 `d830cb9a5a482cfccbd6c963f0f3d1ff0149833fbaf8745238838f13a94b99b8`, contains no stale detour fragment; the replacement branch is one straight `42.38 mm` segment with 0 bends and detour ratio `1.0`. GND/VCC are exactly unchanged; 5 parts, 7 pins, 3 nets, 4 wires and endpoint counts `GND=2`, `OUT=3`, `VCC=2` remain. ERC/connectivity findings are zero; five review warnings are only unchanged missing manufacturer/MPN metadata.
- Exact 21-call public-MCP build/repair evidence `c12_01_e.json`, SHA-256 `4466016e2837458e5a3d9465185709e1cfb1de79839d3041d0588ceb182478fe`. Two harness starts were **INVALID ATTEMPT** only: one omitted local `PYTHONPATH`; one used exact equality instead of the established `1e-6 mm` native-coordinate tolerance. Both stopped during evidence assertions and the guarded artifacts were rebuilt from the verified seed. Focused operator visual verdict: "Хорошо выглядит". No production code or public contract changed.
- Native Save left candidate `01` byte-identical. Close/Reopen and re-export `c12_rt01.dchxml` succeeded, SHA-256 `856f27fd9ddc4ca3a4c4fd1b0119907fa7055d02e0a56c191c5c481cfe9f5cdf`. The straight repaired OUT branch, all four wire geometries, unchanged GND/VCC and exact endpoint counts survived; ERC/connectivity findings remain zero.
- Final semantic comparison passes all 12 required schematic categories with no differences, missing/unsupported categories or parse warnings; disclosed native normalizations: `coordinate_precision`, `whitespace_in_text`. Evidence `c12_ev01.json` SHA-256 `154d36a17858b46cb6753a12a35dde121e516c9a26e4d856b705740e1fbceb69`; manifest SHA-256 `d50d8c9971f311fad1d809cbe832d87233e1c33927a766b0ccb2bb7f66aa64d6`. Authority remains `user_supplied`, `grants_high_trust=false`.
- Final classification: **PASS** — explicit planner feedback correctly identified the electrically safe but excessive pre-existing detour, one atomic public repair removed it without stale fragments or neighboring-net changes, visual quality was operator-accepted and native round-trip preserved all semantics. No production code changed; cases 01–11 and unrelated gates remain valid. Next meaningful case: `13 true fail-closed unresolved-connectivity case`.

### 13 true fail-closed unresolved-connectivity case

- Base code identity `abf803f`; the native-canonical case-12 re-export is copied through public MCP. Controlled public setup disconnects the OUT-port and R1 pins, leaving OUT with one resolvable endpoint and two explicit `schematic.unconnected_pin` findings. Fixture `c13_u.dchxml` SHA-256: `0e65f2d4f486a45718948c5b41cc3329f55e8ff409ebd17c986e7b0d0adafd30`.
- The real public failure attempt is one guarded transaction containing `delete_wire x2 -> move_components -> add_wire`; its replacement references the now-unresolved R1 pin. `stage_operations` refuses before transaction-state update, then explicit rollback marks the uncommitted transaction `rolled_back` with `document_restored=false`. Source SHA, bytes, semantic fingerprint and the two pre-existing connectivity findings remain exactly unchanged. No partial wire deletion, component move or new wire reaches the artifact, so the core fail-closed safety behavior passes and no native-host round-trip is applicable.
- A product defect was found in the initial refusal response: semantic compiler correctly raised domain `connectivity_conflict`, but the common public boundary lacked that mapping and returned `INTERNAL_ERROR`, `retryable=false`. The pre-fix attempt is **SEMANTIC FAIL** for error semantics, while its mutation safety still passed. Captured summary `c13_pre_fix_summary.json` SHA-256 `e085ebc9cb8e35dd798df17f7173f5ed5d9a1f334f1b366ce9bb86aea829b21a`; original pre-fix evidence SHA-256 `eb786f68672804db36afa24fb5103fcb910753855beaf3711bb3edab0b3bdc86`.
- Minimal root repair maps existing domain `connectivity_conflict` to the already-public `CONFLICT` taxonomy entry; no tool, field, parameter or public error code was added. Production/test diff SHA-256 `779787fb7320b933c5d4edff9c36e19599354b201f1135a4ea9c333ad73570dd`. One focused regression asserts `CONFLICT`, `retryable=true`; 9 affected boundary/add-wire/atomic-unresolved/uncommitted-rollback tests pass, Ruff and `git diff --check` pass.
- Exact repaired 15-call public-MCP evidence `c13_01_e.json` SHA-256 `bae15c0a0dc030d5f774645e70f5665567e332b2086046b7e09c923a23409c13`. The refusal is now `CONFLICT`, `retryable=true`, preserves the safe message and stable object id, and all fail-closed byte/semantic checks still pass.
- Final classification: **PASS** on the repaired worktree — true unresolved connectivity refuses atomically with a correct actionable public error and zero artifact mutation. Impact is limited to boundary classification of this existing domain error; the exact affected MCP retest is complete and the unit test is the adjacent smoke. Cases 01–12 and unrelated manual gates remain valid. Next meaningful case: `14 move/rotation/connectivity interaction`.

### 14 move/rotation/connectivity interaction

- Code identity `fd86931`; native-canonical case-12 re-export is reused because case 13 intentionally produced no valid changed artifact. The case moves all five divider parts, rotates R1 by 90 degrees from the seed's native vertical `270°` to horizontal `0°`, deletes four old wires and authors four replacements in one guarded public transaction.
- Candidate `c14_01.dchxml`, SHA-256 `ef82892af16c76f63414b06199f2ebd839742b7c5435a56105b8eb64510962a5`, is **INVALID ATTEMPT**: the harness incorrectly assumed the visually vertical native resistor started at `0°` and requested absolute `90°`, leaving it vertical with reversed pins. The resulting cleaner correctly followed the actual rotated pin geometry, so this is not a product routing defect and native-host review was skipped.
- Candidate `c14_02.dchxml`, SHA-256 `57f704051e941b78cb291e84e59975e86fbc9648caeae120ebf99375e407a0fa`, requests the correct absolute `0°`. One transaction stages/validates/commits `delete_wire x4 -> move_components x5 -> rotate_components -> add_wire x4`. The result retains 5 parts, 7 pins, 3 nets, 4 straight orthogonal wires and endpoint counts `GND=2`, `OUT=3`, `VCC=2`; no old wire fragment remains. Six resolvable resistor/VCC pin attachments match the final wire endpoints within `1e-6 mm`. GND/OUT NetPort geometry remains explicitly unresolved by the embedded-cache matcher, but their public authored anchors and topology are present. ERC/connectivity findings are zero; five review warnings are only unchanged missing manufacturer/MPN metadata.
- Exact 11-call public-MCP evidence `c14_02_e.json`, SHA-256 `5fa2c9448bdf607579578d75f2208e9f7a2659f98d7429ec2c4d4781da2ac009`. The first evidence pass on candidate `02` is an **INVALID ATTEMPT** only: it incorrectly required the two known geometry-unresolved NetPort types to appear in the exact embedded-pin matcher. The guarded rebuild is byte-identical. Operator verdict: "Чисто принципиально это будет работать, как проверка вращения резистора, ок"; visual gate is accepted within the explicit move/rotation/connectivity scope.
- Native Save, Close/Reopen and re-export `c14_rt02.dchxml`, SHA-256 `366f1722e4acb61b4b8afdec09f4b35ed8686849057c540a22673f8001468d06`, preserve the component/pin/net/wire counts, endpoint membership, R1 rotation and all six resolvable physical pin attachments; ERC/connectivity findings remain zero. The first round-trip harness run is **INVALID ATTEMPT** only because its `1e-6 mm` source-position tolerance was stricter than the project's established four-decimal semantic coordinate normalization. Observed native quantization is at most `0.0000184 mm`; rerun under the existing policy passes all 12 compared categories with no differences, only `coordinate_precision` and `whitespace_in_text` normalizations. Evidence `c14_ev02.json` SHA-256 `6f4058d083070ebddecc5f0d241b5267a42cc1912b1ba907fefa4dead1207b68`; recorded round-trip evidence SHA-256 `325ecbf0f7a5c07f01efb93cf83bcb0b2ae94f85254b9db915e6a14f7e9e1d08`.
- Final classification: **PASS** on `fd86931`. Rotation plus movement keeps intended connectivity and readable geometry through native round-trip. No production code changed, so cases 01–13 remain valid and no adjacent manual retest is needed. Next meaningful case: `15 incremental editing`.

### 15 incremental editing

- Code identity `d0d26b7`; seed is the native-canonical case-14 re-export `c14_rt02.dchxml`, SHA-256 `366f1722e4acb61b4b8afdec09f4b35ed8686849057c540a22673f8001468d06`. Candidate `c15_01.dchxml`, SHA-256 `b3ef83a90ff3e913e33d65ac9bcf6dcf3bb927f5a2aef9e8535b127da9e14e15`, is built by seven separate guarded public writes: set R1 to `22k`; place vertical R3 `100k`; place its own downward GND port; connect R3 to OUT and GND in two steps; add the two physical branch wires in two more steps.
- All eight recorded artifact states have distinct SHA-256 values and each write uses the immediately preceding hash as its CAS guard. All five original parts retain their position/rotation, non-R1 values remain unchanged, and all four original wire records remain byte-equivalent at the public-model level after every step. Final topology is 7 parts, 10 pins, 3 nets and 6 wires with endpoint counts `GND=4`, `OUT=4`, `VCC=2`; the two new wires are straight vertical branches terminating at the existing OUT endpoint and the new GND symbol. ERC/connectivity findings are zero; seven review warnings are only missing manufacturer/MPN metadata.
- Exact 19-call public-MCP evidence `c15_01_e.json`, SHA-256 `871372da15e274459fe8f3721627fe80cb71ff21d43690acbe55143bc421cdfa`. Operator verdict: "Просто появился +1 резистор, выглядит нормально"; the operator also correctly notes that the added load is of little practical value. This functional artificiality is accepted only because the explicit case scope is preservation across incremental edits, not realistic circuit design.
- Native Save, Close/Reopen and re-export `c15_rt01.dchxml`, SHA-256 `619009fcab96a11d0bea87bf2fa5cb5f18c6f05de77391d347a5145b0a2b4b99`, preserve all counts, endpoint memberships, `22k/100k` values, R3 rotation and six straight wires; ERC/connectivity findings remain zero. All 12 semantic categories pass with no differences; only `coordinate_precision` and `default_attribute_omission` normalizations are ignored. Evidence `c15_ev01.json` SHA-256 `6ba6be77310821e2b0c59c402f60cb2ded9bf40ad31ffbf1ee9913d34f0f64ab`; recorded manifest SHA-256 `5e34ef778832b4043bcceb1b3e3c9db3ceba69c809d2d6fa75972e28b2dde7ce`.
- Final classification: **PASS** on `d0d26b7` for incremental editing. Seven independently committed guarded writes preserve prior state and survive the native round-trip. No production code changed, so cases 01–14 remain valid and no adjacent manual retest is needed. Next meaningful case: `16 failed-operation/transaction safety`.

### 16 failed-operation/transaction safety

- Base code identity `487ae26`; disposable `c16_01.dchxml` is a byte-identical public-MCP copy of the native-canonical case-15 re-export, SHA-256 `619009fcab96a11d0bea87bf2fa5cb5f18c6f05de77391d347a5145b0a2b4b99`. This intentionally exercises a different failure phase from case 13: three valid operations (`set_component_value`, `move_components`, `delete_wire`) are staged and validated, then commit receives a syntactically valid but deliberately wrong confirmation SHA-256.
- Core safety behavior passes both before and after repair: public commit returns retryable `CONFLICT`; transaction remains `validated`; target bytes, SHA-256, semantic fingerprint, ERC and connectivity remain exactly unchanged. Explicit rollback leaves the uncommitted artifact untouched, and a fresh adjacent transaction on the same file subsequently stages, validates and rolls back successfully. No native-host gate applies because the intentional failure produces no changed artifact.
- A public-diagnostic defect was found: the pre-fix response said `Document changed` even though its own details showed `current_sha256 == transaction_expected_sha256`; only `provided_sha256` was wrong. The failure response is therefore a **SEMANTIC FAIL** for actionable error semantics, while mutation safety remains PASS. Pre-fix 15-call evidence `c16_pre_e.json` SHA-256 `d39ee435f36fb459acc2d5faf33fab070056dd85110ea6a196abf885936512ae`.
- Minimal root repair replaces the two unconditional, misleading commit/rollback messages with cause-neutral SHA mismatch diagnostics. No tool, parameter, field, error code or retry behavior changes. Production/test diff SHA-256 `8fd03fe4a7156dd645020bb57561dc1a28c2b9a37de14bb7e91676d9f911f0c8`; the existing transaction test now covers both wrong-confirmation branches. Seven affected transaction/error-boundary tests pass, Ruff and `git diff --check` pass.
- Exact repaired 15-call public-MCP evidence `c16_01_e.json`, SHA-256 `9c40385cd116e9d035ae682d1fede99156cac0259699f0f58959c2e14c8b8c16`, returns `Commit refused: SHA-256 confirmation or current document mismatch` with `CONFLICT`, `retryable=true`, accurate three-hash details and unchanged fail-closed evidence.
- Final classification: **PASS** on the repaired worktree. The affected manual surface is only failed commit/rollback diagnostics; exact commit retest and adjacent rollback unit smoke are complete. Cases 01–15 and their native evidence remain valid. Next campaign item: `17 representative native open/save/reopen/re-export`; existing current representative evidence should be reused unless impact analysis identifies a missing gate.

### 17 representative native open/save/reopen/re-export

- **PASS by impact-based evidence reuse.** Case 15 already supplies a current representative authored/modified schematic, positive operator visual verdict, real DipTrace Save, Close/Reopen and re-export, followed by a complete 12-category semantic comparison with zero differences, zero ERC findings and zero connectivity findings. Source/re-export and evidence hashes are recorded in case 15 above.
- The only subsequent production change in `d82f5cb` changes two failed-transaction error messages; it does not touch parsing, authoring, placement, routing, serialization, evidence comparison or native artifacts. Therefore no new real-host action or adjacent smoke is justified, and the case-15 native evidence remains valid on the current code identity. Next meaningful case: `18 larger 15–30 component stress schematic`.
