...
- Root repair sets the existing Wire endpoint `BusN` attribute to the containing native net id in the shared `add_wire` serializer. Pin and Free endpoint serialization is unchanged; no public MCP contract changed. Regression asserts the official Wire endpoint structure, and 65 affected schematic authoring/routing tests plus Ruff and `git diff --check` pass.
- Impact-based retest: rebuild only case-08 candidate `04`; visually it should match accepted candidate `03`, so one focused visual confirmation followed by Save/Close/Reopen/re-export is sufficient. The adjacent Pin-to-Pin smoke is automated. Cases 01–07 and unrelated gates remain valid. Case `09 atomic reroute — one net` remains next only after case 08 closes.
