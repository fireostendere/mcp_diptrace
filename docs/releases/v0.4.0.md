...
- Public redownload/PyPI clean-install verification run: `31979666091`.
...
A separate GitHub-hosted Ubuntu runner then performed an independent public-path
verification in run `31979666091`: it downloaded all four assets from the public
`v0.4.0` release URLs, recomputed and matched every recorded SHA-256, checked the
public `SHA256SUMS.txt`, queried public PyPI JSON and matched both distribution
digests, then clean-installed `diptrace-mcp==0.4.0` from `pypi.org/simple` with
`--no-cache-dir` and passed CLI/version smoke. The reusable release workflow also
runs this public verification weekly to detect later availability or integrity
regressions without changing published bytes.
