...
def test_catalog_library_pattern_selectors_and_validation(tmp_path: Path) -> None:
    ...
    assert builtin._component_row(location, "builtin-component:1234:0")["name"] == "BSS138"
    ...
    with sqlite3.connect(database) as connection:
        connection.execute(
            """
            INSERT INTO Components VALUES(
                2, 0, 1, 1, 1, 'MOSFET', 'BSS138', '', 'Q', 'SOT23', '', 'other',
                '', '', '', 'SMD', 'duplicate')
            """
        )
...
<Pattern Style="Missing"/><Pins><Pin Id="0"/></Pins></Part></Component></Components>
    ...
    with pytest.raises(DocumentError, match="not exported"):
        builtin._component_definitions(
            missing_pattern, schematic, {"library_index": 0, "name": "BSS138"}
        )
...
def test_library_export_validation_and_top_level_lifecycle(
    ...
    assert headless_gui._validated_library_export(root, source, target)[1] == source.resolve()
    ...
    class Worker:
        ...
        def wait(self, _timeout: float):
            ...
            result_path.write_text(str(state["payload"]).replace("'", '"'), encoding="utf-8")
            return 0
...
def test_native_menu_dialog_wait_and_worker_helpers(
    ...
    with pytest.raises(headless_gui.HeadlessGuiError, match="timed out"):
        headless_gui._wait_for_export(
            SimpleNamespace(windows=lambda **_kwargs: []), tmp_path / "missing.elixml", 0
        )
...
def test_perform_library_export_worker_success_and_failure(
    ...
    monkeypatch.setattr(headless_gui, "_visible_dialog", lambda *_args: SimpleNamespace(handle=88))
    ...
    monkeypatch.setattr(
        headless_gui,
        "_main_window",
        lambda *_args: (_ for _ in ()).throw(headless_gui.HeadlessGuiError("boom")),
    )
...
def test_cmd_library_export_worker_identity_guards(
    ...
    request.write_text(
        '{"diptrace_root":"r","source":"s","target":"t",'
        '"timeout_seconds":2,"_expected_window_station":"WinSta0",'
